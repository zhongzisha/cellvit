...
class CellPostProcessor:
    ...
    def _remove_overlap(self, cleaned_edge_cells: pd.DataFrame) -> pd.DataFrame:
        ...
        for iteration in range(20):
            ...
            tree = strtree.STRtree(list(poly_dict.keys()))

            merged_idx = deque()
            iterated_cells = set()
            overlaps = 0

            for query_poly, uid in poly_dict.items():
                if uid not in iterated_cells:
                    ...
                    if (
                        ...
                        for inter_poly in intersected_polygons:
                            if inter_poly not in poly_dict:
                                continue
                            uid1 = poly_dict[inter_poly]
                            if (
                                uid1 != uid
                                and uid1 not in iterated_cells
                            ):
                                if (
                        ...
                        if len(submergers) == 0:
                            ...
                            selected_poly_uid = poly_dict[submergers[selected_poly_index]]
                            merged_idx.append(selected_poly_uid)
                    else:
                        # no intersection, just add
                        merged_idx.append(uid)
                    iterated_cells.add(uid)
